...
async def reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if str(update.effective_chat.id) in CONFIG["telegram_bot"]["chats"]:
        response = ask(update.message.text)
        await update.message.reply_text(response, reply_to_message_id=update.message.id)
    else:
        await update.message.reply_text(
            "Please talk to me in Dabney Main or in 24/7 Real Comptroller Help. I wil not respond in other chats."
        )
...
def main() -> None:
    ...
    application = Application.builder().token(CONFIG["telegram_bot"]["token"]).build()
